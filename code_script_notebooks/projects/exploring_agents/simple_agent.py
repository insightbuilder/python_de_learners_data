...
# app_type = input("Provide app type: ")
# app_name = input("Provide app name: ")
# prompt = input("Provide the prompt: ")
app_type = "web api"
...
ask_tasks = False
file_name = 'agent_steps.json'
class_tasks = False
file_task_classified = "agent_tasks_classified.json"  # create a new file
analyse_data = False
file_task_analysed = "agent_tasks_analysis.json"  # create a new file
gen_code = True
file_code_task = "agent_code_output.json"
...
if ask_tasks:
    ...
    # update the tasks to context
    # split_tasks = [task for task in split_tasks if ("###" not in task)
                    # and (task != "")]
    context['tasks'] = split_tasks
    if file_name in os.listdir("."):
        os.remove(file_name)
    # write the data to the json file that keeps track of agent steps
...
if class_tasks:
    ...
    if file_task_classified in os.listdir("."):
        os.remove(file_task_classified)  # ensure the existing file is removed
...
if analyse_data:
    ...
    question = input("Provide your question below: ") 
